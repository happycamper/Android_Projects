...
//  Serial.println(x);
  ...
  for(int i=0;i<16;++i){
      ...
      phases[i]-=(float)360*((int)phases[i]/360);
            ...
            setpotvalue(8,phases[8]);

            setpotvalue(9,phases[9]);

            setpotvalue(10,phases[10]);

            setpotvalue(11,phases[11]);

            setpotvalue(12,phases[12]);

            setpotvalue(13,phases[13]);

            setpotvalue(14,phases[14]);

            setpotvalue(15,phases[15]);
   ...
   Serial.println("pots written");
                ...
                Serial.print(x);Serial.print(",");Serial.print(checksum);Serial.print("\n");
                  execute_write(payload[0],payload[1]);
             }
...
void execute_write(int xx, int yy){
  
  int x = 0;
  int y = 0;
                   x=xx; //reuse memory variables
 ...
 //Serial.println(phases[8]);
 

            setpotvalue(0,phases[8]);

            setpotvalue(1,phases[9]);

            setpotvalue(2,phases[10]);

            setpotvalue(3,phases[11]);

            setpotvalue(4,phases[12]);

            setpotvalue(5,phases[13]);

            setpotvalue(6,phases[14]);

            setpotvalue(7,phases[15]); 
  Serial.println("set pots");
